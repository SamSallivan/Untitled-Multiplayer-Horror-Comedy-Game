...
public class OccludeAudio : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!overridingLowPass)
		{
			if (occluded)
			{
				lowPassFilter.cutoffFrequency = Mathf.Lerp(lowPassFilter.cutoffFrequency, Mathf.Clamp(2500f / (Vector3.Distance(GameSessionManager.Instance.audioListener.transform.position, base.transform.position) / (thisAudio.maxDistance / 2f)), 900f, 4000f), Time.deltaTime * 8f);
			}
			else
			{
